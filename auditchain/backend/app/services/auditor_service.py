...
async def get_by_id(db: AsyncSession, id: uuid.UUID) -> AuditorOut:
    ...
    if auditor is None:
        raise HTTPException(
            ...
            detail=f"Auditor {id} no encontrado",
        )
...
async def create(db: AsyncSession, schema: AuditorCreate) -> AuditorOut:
    ...
    if usuario is None:
        raise HTTPException(
            ...
            detail=f"Usuario {schema.usuario_id} no encontrado",
        )
    if usuario.rol != "auditor":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="El usuario referenciado debe tener rol='auditor'",
        )
    if not usuario.activo:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="El usuario referenciado está inactivo",
        )
    ...
    if existing is not None:
        raise HTTPException(
            ...
            detail=f"Ya existe un auditor para el usuario {schema.usuario_id}",
        )

    auditor = Auditor(usuario_id=schema.usuario_id)
    db.add(auditor)
    await db.flush()
