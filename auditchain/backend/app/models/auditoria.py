...
class Auditoria(Base):
    ...
    sucursal_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("auditchain.sucursales.id", ondelete="CASCADE")
    )
    auditor_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("auditchain.auditores.id", ondelete="SET NULL"), nullable=True
    )
    ...
    estado: Mapped[str] = mapped_column(String(30), server_default="pendiente")
